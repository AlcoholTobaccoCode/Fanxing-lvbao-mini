<template>
	<view class="sv-editor-wrapper" @longpress="eLongpress">
		...
		<editor
			...
			@blur="onEditorBlur"
		></editor>
		<view class="maxlength-tip" v-if="maxlength > 0 && !hideMax"
			>{{ textlength }}/{{ maxlength }}</view
		>
...
export default {
	...
	mounted() {
		...
		// 初始化 readOnly 状态,确保 prop 的初始值正确同步到 store
				...
				setTimeout(() => {
					this.editorCtx.setContents({
						...
						success: () => {
							...
							this.editorCtx.changeInput();
							// #ifdef APP || H5
							if (this.pasteMode == "origin")
								this.editorCtx.changePasteMode("origin");
							// #endif
						}
					});
				});
