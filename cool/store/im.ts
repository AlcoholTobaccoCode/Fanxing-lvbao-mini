...
export class ImStore {
	...
	private eventHandlers = {
		onTextMessage: null as ((msg: OnTextMsgResult) => void) | null,
		onSendMsg: null as ((res: any) => void) | null,
		onReadStatusUpdated: null as ((payload: any) => void) | null,
		onLogin: null as (() => void) | null
	};
	...
	private setupEventListeners() {
		...
		this.eventHandlers.onTextMessage = (msg: OnTextMsgResult) => {
		...
		imBus.on("im:onTextMessage", this.eventHandlers.onTextMessage);

		// 监听发送消息成功 - 更新本地缓存（未读数不变）
		this.eventHandlers.onSendMsg = (res: any) => {
		...
		imBus.on("im:onSendMsg", this.eventHandlers.onSendMsg);

		// 监听已读状态更新 - 直接减少本地缓存的未读数
		this.eventHandlers.onReadStatusUpdated = (payload: any) => {
			console.log("[IM Store] 消息已读，更新本地缓存:", payload.peerId);
			this.updateConversationReadStatus(payload.peerId);
		};
		imBus.on("im:onReadStatusUpdated", this.eventHandlers.onReadStatusUpdated);

		// 监听登录成功 - 仅此时调用一次接口获取初始数据
		this.eventHandlers.onLogin = () => {
			console.log("[IM Store] 登录成功，初始化会话列表...");
			this.isInitialized.value = true;
			this.loadConversations();
			this.initAudio();
		};
		imBus.on("im:onLogin", this.eventHandlers.onLogin);
	}
	...
	/**
	 * 销毁 IM Store，移除所有事件监听器
	 * 在用户登出或应用销毁时调用
	 */
	destroy() {
		// 移除事件监听器
		if (this.eventHandlers.onTextMessage) {
			imBus.off("im:onTextMessage", this.eventHandlers.onTextMessage);
			this.eventHandlers.onTextMessage = null;
		}
		if (this.eventHandlers.onSendMsg) {
			imBus.off("im:onSendMsg", this.eventHandlers.onSendMsg);
			this.eventHandlers.onSendMsg = null;
		}
		if (this.eventHandlers.onReadStatusUpdated) {
			imBus.off("im:onReadStatusUpdated", this.eventHandlers.onReadStatusUpdated);
			this.eventHandlers.onReadStatusUpdated = null;
		}
		if (this.eventHandlers.onLogin) {
			imBus.off("im:onLogin", this.eventHandlers.onLogin);
			this.eventHandlers.onLogin = null;
		}

		// 清空数据
		this.clear();

		console.log("[IM Store] 事件监听器已移除，Store 已销毁");
	}
